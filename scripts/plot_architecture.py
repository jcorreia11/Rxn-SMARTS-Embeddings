...
def box(ax, x, y, w, h, color, label, sublabel=None, fontsize=9, radius=0.05):
    ...
    ax.text(x, y + dy, label, ha="center", va="center",
            fontsize=fontsize, fontweight="bold", color=C_EDGE, zorder=4)
    if sublabel:
...
def token_box(ax, x, y, label, masked=False, special=False):
    ...
    rect = FancyBboxPatch(
        ...
        linewidth=1.0, edgecolor=C_EDGE, facecolor=color, zorder=3,
    )
    ax.add_patch(rect)
    ax.text(x, y, label, ha="center", va="center",
            fontsize=7.5, color=C_EDGE, family="monospace", zorder=4)
...
# ── title ────────────────────────────────────────────────────────────────────
ax.text(FIG_W / 2, 11.6, "SmartRxnEmbeddings — Model Architecture",
        ha="center", va="center", fontsize=12, fontweight="bold", color=C_EDGE)

# ── 1. Input SMARTS string ───────────────────────────────────────────────────
ax.text(FIG_W / 2, 11.15, "Input Reaction SMARTS", ha="center", va="center",
        fontsize=9, color="#555555")
smarts_str = "[C;H0:1](=[O:2])-[OH]>>[C;H0:1](=[O:2])-[O-]"
ax.text(FIG_W / 2, 10.78, smarts_str, ha="center", va="center",
        fontsize=8, color=C_EDGE, family="monospace",
...
ax.text(FIG_W / 2, 10.12, "Tokenizer  (rule-based, 4 465 types)", ha="center",
        va="center", fontsize=8.5, color="#555555",
        style="italic")

for i, (xi, tok) in enumerate(zip(xs, tokens)):
    token_box(ax, xi, y_tok, tok,
              masked=(i in masked), special=(i in specials))

# ── 3. Embedding block ───────────────────────────────────────────────────────
arrow(ax, FIG_W / 2, y_tok - 0.2, FIG_W / 2, 9.18)
box(ax, FIG_W / 2, 8.88, 5.6, 0.52, C_EMBED,
...
box(ax, FIG_W / 2, y_hidden, 5.6, 0.44, "#D5D8DC",
...
# fork arrows
ax.annotate("", xy=(x_left,  y_branch + 0.18), xytext=(FIG_W / 2, y_fork),
            arrowprops=dict(arrowstyle="-|>", color="#E74C3C", lw=1.3,
                            connectionstyle="arc3,rad=0.15"), zorder=5)
ax.annotate("", xy=(x_right, y_branch + 0.18), xytext=(FIG_W / 2, y_fork),
            arrowprops=dict(arrowstyle="-|>", color="#1A5276", lw=1.3,
                            connectionstyle="arc3,rad=-0.15"), zorder=5)

# branch labels
ax.text(x_left,  y_branch + 0.54, "Pre-training", ha="center",
        fontsize=8, color="#E74C3C", fontstyle="italic")
ax.text(x_right, y_branch + 0.54, "Inference", ha="center",
        fontsize=8, color="#1A5276", fontstyle="italic")

# MLM head
box(ax, x_left, y_branch, 3.6, 0.52, C_HEAD,
    "MLM Head",
    sublabel="Linear → GELU → LayerNorm → Linear (4 465)",
    fontsize=9)

# Pooling block
box(ax, x_right, y_branch, 3.6, 0.52, C_POOL,
    "Pooling",
...
box(ax, x_left,  y_out, 3.0, 0.3, "#FADBD8",
    "Vocab logits  (4 465)",  fontsize=8.5)
box(ax, x_right, y_out, 3.0, 0.3, "#FEF9E7",
    "Reaction embedding  (256-d)",  fontsize=8.5)
...
legend_items = [
    ...
    mpatches.Patch(facecolor="#D5D8DC", edgecolor=C_EDGE, label="Special token ([BOS])"),
...
ax.legend(handles=legend_items, loc="lower right",
          fontsize=7.5, framealpha=0.9, edgecolor="#BDC3C7",
